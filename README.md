...
在“代理池与任务设置”抽屉中配置巡检批次和周期后启动。每轮会优先选择最近最久未检查的账号，按批次执行额度查询；遇到 401 时，如果勾选“发现 401 后立即密码 + 2FA 重登录”，会在同一批次中立即启动重登录并保存新凭证。巡检配置会保存到本工具数据库，服务重启后如果之前处于运行状态会自动恢复。

### 自动推送号池

同一个抽屉中可以配置 CPA 和 Sub2API，并开启“自动推送”。推送完全由浏览器直接请求目标号池：地址、密钥不会发送到本工具后端，也不会写入本工具 SQLite。自动巡检或手动重登录成功后，浏览器检测到新凭证会按队列推送；也可以选择账号后点击“推送选中账号”。目标号池需要允许当前页面来源的 CORS 请求。
